...
namespace In
{
    ...
    IN getin(wchar_t infile[])
    {
        ...
        unsigned char* textWithoutSpaces = new unsigned char[IN_MAX_LEN_TEXT];//Выделение памяти для нового массива без повторяющихся пробелов
        int j = 0;
        ...
        for (int i = 0; i < in.size; i++)
        {
            ...
            {
                textWithoutSpaces[j] = inText[i];//копируется вновый массив
                j++;
            }
        }
