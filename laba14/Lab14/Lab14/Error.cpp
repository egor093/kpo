...
namespace Error
{
	...
	ERROR geterror(int id) //принимает идентификатор ошибки id и возвращает соответствующую ошибку из массива .
	{
		if (id > ERROR_MAX_ENTRY - 1 || id < 1)
			return errors[0];
		return errors[id];
	}
	ERROR geterrorin(int id, int line = -1, int col = -1)//принимает идентификатор ошибки id, номер 
		//строки line и номер колонки col и возвращает соответствующую ошибку 
		// из массива errors, дополнительно устанавливая значения строки и колонки в структуре ошибки.
	{
		if (id > ERROR_MAX_ENTRY - 1 || id < 1)
			return errors[0];
		else
